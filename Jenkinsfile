pipeline {
    ...
    environment {
    DOCKERHUB_CREDENTIALS = credentials('anne-dockerhub')
    APP_NAME = "annemariegainsom/blue-green"
    }
    stages { 
        stage('SCM Checkout') {
            steps{
           git branch: 'main', url: 'https://github.com/Amgbillion/argo-rollout-nodejs.git'
            }
        }
        // run sonarqube test
        stage('Run Sonarqube') {
